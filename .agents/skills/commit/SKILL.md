...
following this repo's workflow rules (see `AGENTS.md`). Invoke as `/commit`,
...
single `Agent` call with `subagent_type: "Codex"` and `model: "haiku"`,
...
- Exception: `.Codex/` and `graphify-out/` are deliberately committed in
