-- Prise de commande depuis la page QR (client anonyme). Toute la commande
-- passe par une fonction SECURITY DEFINER : aucune policy INSERT anonyme sur
-- orders/order_items. Les prix et suppléments d'options sont lus et figés
-- côté base — jamais fournis par le client — donc impossible de falsifier un
-- montant. Complète 20260707000003_rls.sql, qui laissait la prise de commande
-- « hors périmètre ».
...
-- Le client anonyme (page QR) et les membres connectés peuvent commander ;
-- personne d'autre. La fonction est le seul chemin d'insertion de commandes.
revoke execute on function public.place_order(text, int, jsonb) from public;
grant execute on function public.place_order(text, int, jsonb) to anon, authenticated;
