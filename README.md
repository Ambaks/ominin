...
> **Item-level printer routing (2026-09-07):** Kitchen printers can now be configured to print only specific menu items, reducing kitchen noise and improving order routing. New junction table `item_printers` maps items to printers; each printer displays a modal showing its menu categories with checkboxes to select which items it should print. If no items are selected, the printer receives all items (backward-compatible behavior). Backend: `_build_routing()` loads routing rules from `item_printers`, and `_apply_routing()` filters `order_items` before enqueuing jobs; orders that become empty after routing are cancelled. Frontend: new `RoutingModal` component (`frontend/components/gestion/terminaux/routing-modal.tsx`) with category-level toggles and individual item selection; `lib/gestion/terminaux.ts` gained `loadRouting()` and `saveRouting()` mutations to sync with Supabase. Migration `20260907000002_ticket_routing.sql`: creates the junction table with RLS (member read, gérant insert/delete). Verified: backend routing logic tested; frontend component tree-shaking, tsc, lint clean. Cleanup: deleted `TACHES-AMBAKA.md`. **Manual steps**: `supabase db push` the new migration.
