...
**SumUp payment provider** (new): Restaurants can now choose between Stripe and SumUp for table payments on their QR menu. Manager settings page (`/gestion/paiement`) presents both options; selecting one triggers OAuth onboarding specific to that provider. SumUp widget loads on-demand for customer checkout. Database migration `20260817000001_sumup.sql` creates a `sumup_accounts` table (RLS-protected, service_role only) to store OAuth tokens securely — they never transit the browser, matching Stripe Connect security. Backend routes: `/api/sumup/connect` (OAuth flow + status), `/api/sumup/pay` (checkout creation), `/api/sumup/verify` (payment confirmation), `/api/sumup/webhook` (async events). `PaymentProvider` type added to `types.ts`; `setPaymentProvider()` mutation in `api.ts` persists the choice per establishment. Matching Stripe behavior: `onlinePayment` flag gates the feature; provider defaults to Stripe for pre-existing restaurants (backwards compatible). Tested: manager can switch providers and toggle payments independently per provider.
