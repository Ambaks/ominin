...
**VOD-to-Clips pipeline (phases 1–5 complete)**:
- **Phase 1–3** (schema/API/form, pipeline core, review UI + publish bridge) — complete in prior commits.
- **Phase 4 (sensor quality)**: Prosody and chat-velocity sensors now extract speech intensity, pitch, and speech rate from audio via Praat (parselmouth), and emote density with lag correction from chat replays. Shared 10s/0.5s window grid and burst-merge logic factored into `base.py`; all three sensors (prosody, chatsense, semantic) emit aligned 120-point series. Config (`fusion.yaml`) auto-renormalizes fusion weights when chat is absent. Verified: synthetic smoke test with loud pitch burst + lagged emote wall → all sensors fire on the right spans with aligned signal vectors.
- **Phase 5 (frontend polish)**: Job history displays recent analyses on generator idle view; completed jobs reopen review grid, failed jobs show error, running jobs resume progress. Real retry on failures + secondary "Autre vidéo" action. Error states: no more hangs on initial query failure, polling errors inline, clip-list failures show retry, approve/reject failures toast, missing playback URL shows "Aperçu indisponible". Mobile responsive (edge padding, aspect-9/16), fully French (score labels "Débit"/"Chute", stage label "Téléchargement", empty state). New helpers: `fetchRecentJobs()` in API layer, `job-history.tsx` component, `VOD_HISTORY_LIMIT` + `JOB_STATUS_LABELS` constants. Verified: `npm run build` + `npm run lint` pass.
- **Next phase**: real-VOD validation of chat sensors on collaborator worker (currently using smoke tests).

Still deferred: Stripe billing for clip, direct platform APIs, account-creation backend, provider webhooks (polling suffices), DNS/domain setup (Vercel domain binding, CNAME, Supabase redirect URL) — plus one new owner step: create the upload-post account and set `UPLOAD_POST_API_KEY`. Implementation plan for collaborators: `.claude/plans/clip-phase2-espace.md`.
