...
**Coming soon: Account creation from espace**: Comptes tab splits into two sub-modes: "Comptes connectés" (existing flow: manage linked social accounts) and "Créer des comptes" (new, marked "Bientôt"). The account-creation page (`/clip/espace/comptes/creation`) announces the ability to create ready-to-post social accounts directly from the espace — opened, linked, and primed without platform-side configuration. Hero eyebrow, four platform placeholder cards (dashed borders from `CLIP_PLATFORMS`), and an "En attendant" section linking back to connecting existing accounts. The page is static placeholder, ready for backend implementation.
