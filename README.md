...
> **Team member invitations fixed (2026-08-31):** Fixed a bug in the team member invitation flow where Supabase's `generateLink` produces an `action_link` with tokens in the URL hash fragment (`#access_token=...`), which server-side route handlers cannot read. The fix: (1) In the invite API route (`frontend/app/api/gestion/invite/route.ts`), send an email with a link pointing directly to `/auth/callback?token_hash={hashed_token}&type=invite` (query params) instead of using the action_link. (2) In the auth callback (`frontend/app/auth/callback/route.ts`), handle `token_hash` + `type` via `verifyOtp()` to establish the session server-side, keeping the existing code exchange for Google OAuth untouched. The fix is minimal and backwards-compatible — the callback tries verifyOtp first, then falls back to code exchange. Consolidated the separate analytique page into the main gestion dashboard. Verified: `npx tsc --noEmit`, `npm run lint`, and `npm run build` all pass; graphify updated.
