...
> **Outreach scale-up: 200/day, bounce sweep, 14 cities (2026-09-02):** Daily cold email limit raised 100→200 (`outreach_daily_limit`, `outreach_run_batch_size`), enrichment batch 150→300 to keep the qualified pool ahead. New `sweep_bounces()` in `inbox.py` — a regex-only (no Claude calls) bounce detector called at the start of every outreach run: it scans Gmail for mailer-daemon/delivery-status messages, marks the original outbound as failed, adds the address to suppressions, and reverts the lead to `to_contact`, reclaiming daily quota before the compose loop runs. This means bounced slots are automatically backfilled with new sends in the same run. City coverage expanded from 2→14 (Paris, Marseille, Nice, Lyon, Bordeaux, Perpignan, Agen, Lattes, Annecy, Annemasse, Bandol, Le Beausset added to Montpellier+Toulouse). Neighborhoods added for 9 major cities (discovery query matrix). **Manual step**: set `DISCOVERY_NEIGHBORHOODS` on Render with the value from `backend/.env`.
...
`OUTREACH_DAILY_LIMIT` (100), `OUTREACH_MODEL` (`claude-fable-5`),
