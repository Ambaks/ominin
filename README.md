...
6. **GitHub** repo secrets: `BACKEND_URL` (the Render URL, no trailing
   slash) + `AGENT_TRIGGER_SECRET`. The four workflows in
   `.github/workflows/agent-*.yml` (discovery, outreach, inbox, weekly
   autoresearch) then run on schedule; each also has a manual "Run
